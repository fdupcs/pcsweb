...
![下载 TeX 源文件](./assets/latex-environment/1.jpg)

下载完成后，双击 `iso` 镜像文件打开安装包，如果出现下面的界面选择 `打开`：

![安装 TeX 源文件](./assets/latex-environment/2.jpg)

下拉文件找到 `install-tl-windows.bat` 文件，`Shift+鼠标右键` 打开菜单选择 `以管理员身份运行`，进入安装页面：

![运行 TeX 安装文件](./assets/latex-environment/3.jpg)

等待一小段时间进入安装页面后，直接点击右下角的 `安装` 进入安装，安装时间很长，大约需要30-50分钟，请耐心等待：

![TeX 安装向导](./assets/latex-environment/4.jpg)

安装完成后，右下角的 `Abort` 变灰，`关闭` 可以点击，选择 `关闭` 后完成安装，默认情况下会自动添加到环境变量中。

![TeX 安装结束](./assets/latex-environment/5.jpg)

## 配置 VS Code 环境

在 VS Code 商店中搜索 `LaTeX Workshop` 插件选择安装：

![安装 Workshop 插件](./assets/latex-environment/6.jpg)

安装完成后，选择左下角齿轮进入 `设置` 页面，找到 `JSON` 进入页面选择 `在 settings.json 中编辑`：
...
![新建 tex 文件](./assets/latex-environment/9.jpg)
...
此时左边应该出现了 `TEX` 栏，点击绿色运行按钮进行编译文件，点击旁边的按钮查看 PDF，如图所示即成功：

![运行 tex 编译](./assets/latex-environment/10.png)
...
保存 `settings.json` 文件后，在 [https://github.com/OrangeX4/OrangeX4-HyperSnips](https://github.com/OrangeX4/OrangeX4-HyperSnips) 中下载 `latex.hsnips` 和 `markdown.hsnips` 文件，回到 VS Code 按下 `Ctrl+Shift+P` （在 Mac 电脑中为 `Command+Shift+P` ）后输入命令 `Open Snippets Directory` 后打开文件夹，并将前面下载的 `latex.hsnips` 和 `markdown.hsnips` 文件放入文件夹中：
